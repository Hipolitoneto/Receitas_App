...
export default function ReceitasScreen({ navigation }: { navigation: any }) {
  const { user } = useAuth();
  const { getRecipesByUser, deleteRecipe, toggleFavorite, isLoading } = useRecipe();
  
  const userRecipes = user ? getRecipesByUser(user.id) : [];
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            {userRecipes.length === 0 
              ? 'Você ainda não tem receitas. Crie sua primeira receita!' 
              : `${userRecipes.length} receita${userRecipes.length !== 1 ? 's' : ''}`
            }
          </Text>
        </View>
                ...
                <View style={styles.recipeInfo}>
                 ...
                 >
                   <Ionicons 
                     ...
                     size={20} 
                     color={recipe.isFavorite ? '#e74c3c' : '#666'} 
                   />
                 </TouchableOpacity>
